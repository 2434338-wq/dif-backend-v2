...
fun Route.solicitudesRoutes() {
    authenticate("auth-jwt") {
        post("/solicitudes") {
            ...
            val newId = transaction {
                Solicitudes.insertAndGetId {
                    ...
                    it[Solicitudes.imagenUrl] = "no_image.jpg"
        ...
        get("/mis-solicitudes") {
            ...
            val lista = transaction {
                Solicitudes.select { Solicitudes.usuarioId eq userId }.orderBy(Solicitudes.creadoEn, SortOrder.DESC).map { rowToSolicitud(it) }
            }
            call.respond(lista)
        }
    }
    ...
    get("/admin/solicitudes/pendientes") {
        val lista = transaction {
            Solicitudes.select { Solicitudes.estatus eq "Pendiente" }.orderBy(Solicitudes.creadoEn, SortOrder.DESC).map { rowToSolicitud(it) }
        }
        call.respond(lista)
    }
}
